...
if not HISTORY_FILE.get("search_web", "1"):
...
def load_history():
    ...
    for message in HISTORY_FILE["messages"]:
        if message["sender"] == "ai":
            ...
            history.append({"role": "assistant", "reasoning": message.get("reasoning", ""), "content": message.get("answer", "") + (f"{model: >{10+len(model)}}")})
...
def save_history(response: dict = {}, state = None, progress = 0, MODEL: str = "LLM"):
...
def send_message_api(history: list, attempt: int = 0, MODEL: str = "minimax/minimax-m2:free"):
    ...
    try:
        ...
        if follow_message:
            follow_send = send_message_api(history=follow_message, MODEL=MODEL)
        ...
        result = send_message_api(history=history, attempt=(attempt + 1), MODEL=MODEL)
        return result
...
def main():
    for MODEL in ["openrouter/polaris-alpha"]*1:
        time.sleep(1)
        global HISTORY_FILE
        HISTORY_FILE = load_json(HISTORY_PATH)
        logging.info(MODEL)
        try:
            ...
            save_history({}, "start", MODEL=MODEL)
            answer = send_message_api(history=history, MODEL=MODEL)
            if answer.get("fatal_error", ""):
                ...
                save_history(response=answer, MODEL=MODEL)
